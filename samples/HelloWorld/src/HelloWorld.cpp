...
HelloWorld::HelloWorld( const std::wstring& name, int width, int height, bool vSync )
...
, m_FPS( 0.0f )
{
    ...
    m_Logger = GameFramework::Get().CreateLogger( "HelloWorld" );
    ...
    m_Window->Update += UpdateEvent::slot( &HelloWorld::OnUpdate, this );
    m_Window->Resize += ResizeEvent::slot( &HelloWorld::OnResize, this );
    m_Window->DPIScaleChanged += DPIScaleEvent::slot( &HelloWorld::OnDPIScaleChanged, this );
    m_Window->KeyPressed += KeyboardEvent::slot( &HelloWorld::OnKeyPressed, this );
    m_Window->KeyReleased += KeyboardEvent::slot( &HelloWorld::OnKeyReleased, this );
    m_Window->MouseMoved += MouseMotionEvent::slot( &HelloWorld::OnMouseMoved, this );
}
...
bool HelloWorld::LoadingProgress( float loadingProgress )
{
...
bool HelloWorld::LoadScene( const std::wstring& sceneFile )
{
    ...
    auto scene    = commandList->LoadSceneFromFile( sceneFile, std::bind( &HelloWorld::LoadingProgress, this, _1 ) );
...
void HelloWorld::LoadContent()
{
    ...
    m_LoadingTask = std::async( std::launch::async, std::bind( &HelloWorld::LoadScene, this,
...
void HelloWorld::OnUpdate( UpdateEventArgs& e )
{
...
void HelloWorld::OnResize( ResizeEventArgs& e )
{
...
void HelloWorld::OnKeyPressed( KeyEventArgs& e )
{
...
void HelloWorld::OnKeyReleased( KeyEventArgs& e )
{
    if ( !ImGui::GetIO().WantCaptureKeyboard )
    {
        switch ( e.Key )
...
void HelloWorld::OnMouseMoved( MouseMotionEventArgs& e )
{
    if ( !ImGui::GetIO().WantCaptureMouse ) {}
}

void HelloWorld::OnDPIScaleChanged( DPIScaleEventArgs& e )
{
...
void HelloWorld::OnGUI( const std::shared_ptr<CommandList>& commandList, const RenderTarget& renderTarget )
{
    m_GUI->NewFrame();
            ...
            if ( SUCCEEDED( pFileOpen->Show( m_Window->GetWindowHandle() ) ) )
            {
                ...
                if ( SUCCEEDED( pFileOpen->GetResult( &pItem ) ) )
                {
                    ...
                    if ( SUCCEEDED( pItem->GetDisplayName( SIGDN_FILESYSPATH, &pszFilePath ) ) )
                    {
                        ...
                        m_LoadingTask =
                            std::async( std::launch::async, std::bind( &HelloWorld::LoadScene, this, pszFilePath ) );
